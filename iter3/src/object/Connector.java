	...
	public void paintConnector(Graphics g) {
		...
		if (x1 < x2) {
			if (x1 + width + arrowLength <= x2) {
				g.drawLine(x1 + width, y1 + height / 2, x2 - arrowLength, y2 + height2 / 2);
			} else {
				if (y1 >= y2 + height2 + arrowLength) {
					g.drawLine(x1 + width / 2, y1, x2 + width / 2, y2 + height2 + arrowLength);
			...
			if (x1 >= x2 + width + arrowLength) {
				g.drawLine(x1, y1 + height / 2, x2 + width + arrowLength, y2 + height2 / 2);
			} else {
				if (y1 >= y2 + height2 + arrowLength) {
					g.drawLine(x1 + width / 2, y1, x2 + width / 2, y2 + height2 + arrowLength);
				} else if (y1 + height + arrowLength <= y2) {
